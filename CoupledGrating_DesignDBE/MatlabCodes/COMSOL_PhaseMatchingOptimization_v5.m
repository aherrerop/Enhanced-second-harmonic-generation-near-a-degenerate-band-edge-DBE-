...
function cost = objectiveDBE(params, filename1, filename2)
    ...
    try
        ...
        SH_freq = 2*freq_DBE;
        A = find(freq_vec_f2 < SH_freq - 0.3e12);
        B = find(freq_vec_f2 > SH_freq + 0.3e12);
        freq_box = freq_vec_f2(:,A:B);
        kx_box = kx_f2(:,A:B);

        % Get RBE close to 2*freq_DBE
        [numModes, freqLength] = size(freq_box);
        for ii = 1:numModes
            vg_box(ii,:) = diff(freq_box(ii,:)) ./ diff(kx_box(ii,:));
        end
        cost_RBE = min(abs(vg_box),[],'all'); % Minimize GVD


        % Find the index for the frequency closest to 2*freq_DBE
        SH_freq = 2 * freq_DBE;
        tolerance = 1e12;  % Adjust this as needed
